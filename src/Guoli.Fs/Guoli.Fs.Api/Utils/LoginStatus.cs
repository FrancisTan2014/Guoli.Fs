...
namespace Guoli.Bts.Api.Utils
{
    ...
    {
        private static readonly string _tokenName = "app_token";
        ...
        private static Token GetToken(HttpRequest request)
        {
            // 从cookie或者请求参数中获取token
            var token = request.Cookies[_tokenName]?.Value ?? request[_tokenName];
        ...
        /// 判断当前请求的用户是否已登录
        /// </summary>
        public static bool HasLogin(HttpRequest request)
        {
            int userId;
            return HasLogin(request, out userId);
        }

        private static bool HasLogin(HttpRequest request, out int userId)
        {
            userId = 0;
        ...
        {
            int userId;
            if (HasLogin(HttpContext.Current.Request, out userId))
            {
                return ViewSysUserBll.QuerySingle(userId);
            }
